...
$worker = new WORKERS;
$workerslist = $worker->getWorkerList();

$teamlist = $worker->getWorkerTeamList();
$teamName = array();
foreach($teamlist as $value){
	$teamName[$value["teamid"]]=$value["teamName"];
}
...
<center>
	...
	<div align="center">
		...
		<input type="hidden" name="hdnAction" id="hdnAction">
			<br>
		<a href="createworkers.php"><input type="button" name="createworkers" value="Create Workers" class="button"></a>
		<br><br>
		<table cellpadding="1" cellspacing="1" border="0" bgcolor="#EEEEEE" width="100%" class="mediumtxt">
			...
			if(count($workerslist) > 0){
				$i=0;
			foreach($workerslist as $workerval){
                        $i++;
			?>
			<tr id="row_<?php echo $i?>" bgcolor="#FFFFFF">
				<td><?php echo $i?></td>				
				<td><?php echo $workerval["workerName"];?></td>
				<td><?php echo $teamName[$workerval["teamId"]];?></td>				
				<td><?php echo date("d-M-Y",strtotime($workerval["createdOn"]));?></td>
				
				<td><a href="#" onclick='_getBox("editworkers.php?page=Edit&ac=<?php echo $commonObj->Encrypt($workerval["workerId"]);?>","40%","40%")'><img src="images/edit.gif" border="0" alt="edit" title="Edit"></a> &nbsp; &nbsp;<a href="javascript:void(0)" onclick="confimuser('<?php echo $commonObj->Encrypt($workerval["workerId"]);?>','<?php echo $i?>');"><img src="images/close.gif" border="0" alt="Delete" title="Delete"></td>
